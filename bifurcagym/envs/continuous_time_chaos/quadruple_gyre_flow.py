...
class EnvParams:
    ...
    x_bounds: chex.Array = struct.field(False, default=jnp.array((0.0, 2.0)))
    y_bounds: chex.Array = struct.field(False, default=jnp.array((0.0, 2.0)))
    goal_state: chex.Array = struct.field(False, default=jnp.array((1.8, 1.8)))
...
class QuadrupleGyreFlowCSCA(base_env.BaseEnvironment):
    ...
    def step_env(self,
        ...
        new_x = jnp.clip(new_x, params.x_bounds[0], params.x_bounds[1])
    ...
    def _get_flow_velocity(self, x: chex.Array, y: chex.Array, t: float, params) -> Tuple[chex.Array, chex.Array]:
    ...
    def reward_and_done_function(self,
        ...
        dist_to_target = jnp.linalg.norm(jnp.array((state_t.x, state_t.y)) - params.goal_state)
    ...
    def render_traj(self, trajectory_state: EnvState, params: EnvParams, file_path: str = "../animations"):
        ...
        initial_U_coarse, initial_V_coarse = self._get_flow_velocity(X_coarse, Y_coarse, times[0] * params.dt, get_curr_param(params, 0))
        initial_U_fine, initial_V_fine = self._get_flow_velocity(X_fine, Y_fine, times[0] * params.dt, get_curr_param(params, 0))
        ...
        def update(frame):
            ...
            U_coarse, V_coarse = self._get_flow_velocity(X_coarse, Y_coarse, times[frame] * params.dt, get_curr_param(params, frame))
            U_fine, V_fine = self._get_flow_velocity(X_fine, Y_fine, times[frame] * params.dt, get_curr_param(params, frame))
            speed = np.sqrt(U_fine ** 2 + V_fine ** 2)
    ...
    @property
    def name(self) -> str:
        return "QuadrupleGyreFlow-v0"

    def action_space(self, params: EnvParams) -> spaces.Box:
        return spaces.Box(-params.maximum_max_speed, params.maximum_max_speed, shape=(2,), dtype=jnp.float64)

    def observation_space(self, params: EnvParams) -> spaces.Box:
        lo = jnp.array((params.x_bounds[0], params.y_bounds[0]))
        hi = jnp.array((params.x_bounds[1], params.y_bounds[1]))
