...
class PilcoCartPoleCSDA(base_env.BaseEnvironment):
    ...
    def step_env(self,
        ...
        xdot_update = ((-2 * self.mass_pole_length * (state.theta_dot ** 2) * sintheta + 3 * self.masspole * self.gravity
                        * sintheta * costheta + 4 * action - 4 * self.b * state.x_dot) /
                       (4 * self.total_mass - 3 * self.masspole * costheta ** 2))

        thetadot_update = ((-3 * self.mass_pole_length * (state.theta_dot ** 2) * sintheta * costheta + 6 *
                           self.total_mass * self.gravity * sintheta + 6 * (action - self.b * state.x_dot) * costheta) /
                           (4 * self.length * self.total_mass - 3 * self.mass_pole_length * costheta ** 2))
