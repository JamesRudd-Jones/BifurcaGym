...
@pytest.mark.parametrize("env_name, cont_state, cont_action, metrics",
                         all_combinations)

class TestEnv:
    def test_benchmark(self, env_name, cont_state, cont_action, metrics):
        key = jrandom.key(42)
        ...
        batch_key = jax.random.split(_key, num_envs)
        ...
        def update_step(runner_state, unused):
            ...
            if config.get("DEBUG"):
                def callback(info):
                    ...
                    # TODO add a render in here as well, perhaps every so update_steps
